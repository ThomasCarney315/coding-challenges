...
//could have used var here inside for loop to make prefix variable accessable outside the for-loop
function dropWhile(arr, pred) {
    let prefix
    for (prefix = 0; prefix < arr.length; prefix++) {
      if (!pred(arr[prefix]))
        break
    }
    return arr.slice(prefix)
  }