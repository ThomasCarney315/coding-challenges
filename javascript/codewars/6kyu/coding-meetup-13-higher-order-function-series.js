...
function isLanguageDiverse(list) {
    let pyDevs = list.filter(dev => dev.language === 'Python').length;
    let rubyDevs = list.filter(dev => dev.language === 'Ruby').length;
    let jsDevs = list.filter(dev => dev.language === 'JavaScript').length;
    
    return !(pyDevs / rubyDevs > 2 
             || pyDevs / jsDevs > 2 
             || rubyDevs / pyDevs > 2 
             || rubyDevs / jsDevs > 2 
             || jsDevs / pyDevs > 2 
